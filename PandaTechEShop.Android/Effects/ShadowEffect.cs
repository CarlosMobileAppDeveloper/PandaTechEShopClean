...
namespace PandaTechEShop.Droid.Effects
{
    public class ShadowEffect : BaseEffect
    {
        private class ShadowOutlineProvider : ViewOutlineProvider
        {
            ...
            public override void GetOutline(Android.Views.View view, Outline outline)
            {
                ...
                if (button != null)
                {
                    ...
                    outline.SetRoundRect(new Android.Graphics.Rect(0, 0, view.Width, view.Height), (int)num);
                }
                outline.Alpha = PandaTechEShop.Effects.Effects.GetShadowOpacity(_element);
            }
        }
        ...
        {
            if (propertyName == PandaTechEShop.Effects.Effects.ShadowProperty.PropertyName || propertyName == PandaTechEShop.Effects.Effects.ShadowSizeProperty.PropertyName)
            {
                UpdateShadow();
            }
            else if (propertyName == Button.CornerRadiusProperty.PropertyName || propertyName == PandaTechEShop.Effects.Effects.ShadowOpacityProperty.PropertyName)
            {
        ...
        {
            float shadowSize = PandaTechEShop.Effects.Effects.GetShadowSize(base.Element);
            if (!(shadowSize < 0f))
            {
