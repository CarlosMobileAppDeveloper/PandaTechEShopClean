...
namespace PandaTechEShop.Services.RequestProvider
{
    public interface IRequestProvider
    {
        ...
        Task<T> GetAndRetryAsync<T>(string uri, int retryCount, string token = "",
            Func<Exception, int, Task> onRetry = null) where T : class;

        Task<T> GetWaitAndTryAsync<T>(string uri, Func<int, TimeSpan> sleepDurationProvider, int retryCount,
